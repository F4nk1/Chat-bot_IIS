...
.PHONY: help setup db-init db-migrate db-reset run-backend run-frontend build-frontend run test ollama-setup clean
...
ollama-setup: ## Asegura que el modelo phi3 esté descargado en Ollama local
	@echo -e "$(YELLOW)Verificando y descargando el modelo phi3 en Ollama...$(RESET)"
	ollama pull phi3
	@echo -e "$(GREEN)Modelo phi3 listo para usar.$(RESET)"
